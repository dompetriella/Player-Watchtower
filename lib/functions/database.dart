...
Player createFirstCharacterEntry(
...
createNewCharacterEntry(WidgetRef ref) {
  var playerBox = Hive.box<Player>('player');
  var inventoryBox = Hive.box<Inventory>('inventory');
  String guid = Guid.generate().toString();
  Map<String, dynamic> status = {'theme': 'Daylight'};
  Hive.box('characters').put(guid, status);
  Player newPlayer = const Player().copyWith(guid: guid);
  Inventory newInventory = const Inventory().copyWith(guid: guid);
  playerBox.put(guid, newPlayer);
  inventoryBox.put(guid, newInventory);
  ref.watch(playerProvider.notifier).state = newPlayer;
}
...
Future getInitState(WidgetRef ref) async {
  ...
  if (characterBox.isEmpty) {
    ref.watch(playerProvider.notifier).state =
        createFirstCharacterEntry(playerBox, inventoryBox);
  } else {
